...
The panel and island WebViews are created hidden during startup. Hover uses the same `popover.html` panel as the main UI; left click switches that panel into a pinned state. The floating panel is borderless, transparent, clamped to the current monitor work area, keeps a clear 56 px gap from the tray anchor, and includes extra transparent padding so the glass panel shadow and rounded corners are not clipped.
